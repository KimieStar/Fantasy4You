...
{
    public partial class CharacterCustomizationChar2 : Form
    {
        Logic logic = new Logic();
        public CharacterCustomizationChar2()
        {
            InitializeComponent();
        }

        private void CharacterCustomizationChar2_Load(object sender, EventArgs e)
        {
            List<string>[] list1 = logic.SelectCharacterInformation2(2);
            CharacterStrenghtTextbox.Text = string.Join("", list1[0].ToArray());
            List<string>[] list2 = logic.SelectCharacterInformation2(2);
            CharacterDexterityTextbox.Text = string.Join("", list2[1].ToArray());
            List<string>[] list3 = logic.SelectCharacterInformation2(2);
            CharacterConstitutionTextbox.Text = string.Join("", list3[2].ToArray());
            List<string>[] list4 = logic.SelectCharacterInformation2(2);
            CharacterIntelligenceTextbox.Text = string.Join("", list4[3].ToArray());
            List<string>[] list5 = logic.SelectCharacterInformation2(2);
            CharacterWisdomTextbox.Text = string.Join("", list5[4].ToArray());
            List<string>[] list6 = logic.SelectCharacterInformation2(2);
